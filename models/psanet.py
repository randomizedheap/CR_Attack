...
import models.pspresnet as models
...
class PSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shrink_factor != 1:
            ...
            w = (w - 1) * self.shrink_factor + 1
            x = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=True)
        #print("PSA ",out.shape,x.shape)
        return torch.cat((out, x), 1)


class PSANet(nn.Module):
    def __init__(self, num_classes=2,layers=50, dropout=0, zoom_factor=8, use_psa=True, psa_type=2, compact=False,
                 shrink_factor=2, mask_h=None, mask_w=None, normalization_factor=1.0, psa_softmax=True,
                 criterion=nn.CrossEntropyLoss(ignore_index=255),TEST_WIDTH=0,
            TEST_HEIGHT=0, pretrained=False,**_):
        ...
        if mask_h is None and mask_w is None:
            mask_h = 2 * ((TEST_HEIGHT - 1) // (8 * shrink_factor) + 1) - 1
            mask_w = 2 * ((TEST_WIDTH - 1) // (8 * shrink_factor) + 1) - 1
        else:
            assert (mask_h % 2 == 1) and (mask_h >= 3) and (mask_h <= 2 * ((TEST_HEIGHT - 1) // (8 * shrink_factor) + 1) - 1)
            assert (mask_w % 2 == 1) and (mask_w >= 3) and (mask_w <= 2 * ((TEST_HEIGHT - 1) // (8 * shrink_factor) + 1) - 1)
