...
class BaseDataSet(Dataset):
    def __init__(self, root, split, mean, std, base_size=None, augment=True, val=False,
                crop_size=321, scale=True, flip=True, TEST_WIDTH=0,TEST_HEIGHT=0,rotate=False, blur=False, noise=False,return_id=False):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
    ...
    def _val_augmentation(self, image, label):
        ...
        elif max(h,w)==2048:  #Cityscape, use this scale to make sure valid input for PSPNet
            h=513
            w=1025
    ...
    def __getitem__(self, index):
        ...
        if self.base_size:
            ...
            image = cv2.resize(image, (self.TEST_WIDTH, self.TEST_HEIGHT), interpolation=cv2.INTER_LINEAR)
            label = cv2.resize(label, (self.TEST_WIDTH, self.TEST_HEIGHT), interpolation=cv2.INTER_NEAREST)

            #image = cv2.resize(image, (w, h), interpolation=cv2.INTER_LINEAR)
            #label = Image.fromarray(label).resize((w, h), resample=Image.NEAREST)
            #label = np.asarray(label, dtype=np.int32)
            #print('image.size ',image.shape)
            #print('lab.size ',label.size)
