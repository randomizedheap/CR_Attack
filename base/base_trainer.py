...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None,method='normal'):
        self.model = model
        self.loss = loss
        self.config = config
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            if self.method=='normal':
                results = self._train_normal(epoch)
            elif self.method=='noisy':
                results = self._train_noise(epoch)
            else:
